# Aesthetic-Usability Effect

## Summary
Users often perceive aesthetically pleasing design as design that's more usable. This cognitive bias causes people to believe that beautiful things work better, even when they don't.

## Key Takeaways
- An aesthetically pleasing design creates a positive response in people's brains and leads them to believe the design actually works better
- People are more tolerant of minor usability issues when the design of a product or service is aesthetically pleasing
- Visually pleasing design can mask usability problems and prevent issues from being discovered during usability testing
- The effect was first studied in 1995 by researchers Masaaki Kurosu and Kaori Kashimura from the Hitachi Design Center

## Origins/History
The aesthetic-usability effect was first studied in the field of human–computer interaction in 1995. Researchers Masaaki Kurosu and Kaori Kashimura from the Hitachi Design Center tested 26 variations of an ATM UI, asking the 252 study participants to rate each design on ease of use, as well as aesthetic appeal. They found a stronger correlation between the participants' ratings of aesthetic appeal and perceived ease of use than the correlation between their ratings of aesthetic appeal and actual ease of use. Kurosu and Kashimura concluded that users are strongly influenced by the aesthetics of any given interface, even when they try to evaluate the underlying functionality of the system.

## Applications
- Understanding user bias in usability testing sessions
- Balancing aesthetic appeal with functional usability
- Recognizing when beautiful designs may hide usability problems
- Educating stakeholders about the difference between perceived and actual usability
- Informing design review processes to look beyond surface appeal

## See Also
- [Laws of UX](./sources/laws-of-ux-overview.md)
- [Visual Design Principles](./aesthetics/visual-design-principles.md)
- [First Impressions](./concepts/first-impressions.md)
- [Emotional Design](./concepts/emotional-design.md)
- [Cognitive Biases in UX](./concepts/cognitive-bias.md)

## Sources
- [Aesthetic-Usability Effect](./raw/laws-of-ux/aesthetic-usability-effect.md)
- [NN/g Article on Aesthetic-Usability Effect](https://www.nngroup.com/articles/aesthetic-usability-effect/)